  ...
  <div>
    <p style="margin: 20px 0">
      ...
      <el-button @click="pagingScrollTopLeft(5000)"
        >滚动y和x到5千位置</el-button
      >
      <el-button @click="pagingScrollTopLeft(10000)"
        >滚动y和x到1万位置</el-button
      >
...
export default {
  ...
  mounted() {
    ...
    this.LodingUxgrid() // 加载状态
    // this.LodingUxgrid('暂无数据...') // 确实没有数据

  },
  methods: {
    LodingUxgrid(text){
      var emptyDom = document.querySelectorAll(".elx-table--empty-content")
      emptyDom = Array.from(emptyDom)
      for(let i in emptyDom){
        if(emptyDom[i]){
          if(text){
            emptyDom[i].innerHTML = text
          }else{
            let spin = `<div class="ivu-spin ivu-spin-large"><div class="ivu-spin-main"><span class="ivu-spin-dot" style="margin: 0 auto;"></span> <div class="ivu-spin-text"></div></div></div>`
            emptyDom[i].innerHTML = spin
          }
        }
      }
    },
